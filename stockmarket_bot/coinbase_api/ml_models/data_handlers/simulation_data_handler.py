...
class SimulationDataHandler(AbstractDataHandler):
    ...
    def fetch_all_historical_data(self) -> dict[str, pd.DataFrame]:
        ...
        for crypto_model in self.crypto_models:
            ...
            if 'timestamp' not in df.columns or df.empty:
                # print(f'df was empty for: {crypto_model.symbol}')
    ...
    def get_dataframes_subset(self) -> dict[str, pd.DataFrame]:
        ...
        for symbol, df in self.dataframe_cache.items():
            subset[symbol] = df.loc[self.timestamp - timedelta(hours=self.prediction_handler.lstm_sequence_length - 1):self.timestamp]
        return subset
    ...
    def get_crypto_value_from_cache(self, symbol: str, timestamp: datetime) -> float:
        ...
        if timestamp in df.index:
            try:
                ...
                # print(f'Error trying to fetch data for {symbol}: {e}')
                return 0.0
        # print(f'timestamp {timestamp} was not in df.index for {symbol}.')
        return 0.0
    ...
    def get_crypto_account_value(self, crypto_model: AbstractOHLCV) -> Tuple[float, float]:
        try:
            ...
            account_value = 0
        price = self.get_crypto_value_from_cache(crypto_model.symbol, self.timestamp)
    ...
    def get_new_prediction_data(self, timestamp: datetime) -> Dict[str, List[float]]:
        ...
        predictions = Prediction.objects.using(self.database).filter(
        ...
        for prediction in predictions:
            index = 0 if prediction.predicted_field == 'close_higher_shifted_1h' else 1 if prediction.predicted_field == 'close_higher_shifted_24h' else 2
            index += 0 if prediction.model_name == 'LSTM' else 3
            all_entries[prediction.crypto][index] = prediction.predicted_value
